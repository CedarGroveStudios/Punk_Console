...
class PunkConsole:
    ...
    synchronously triggered variable one-shot monostable multivibrator to create
    ...
    the variable inputs. Once the Punk Console class is instantiated, the two
    input values are set and the update() function is called to adjust the PWM
    output pin to to create the output waveform.
    ...
    parameters which can create output signals well above the practical audio
    ...
    Notes:
    Future update: For non-PWM analog pins, the plans are to use audiocore with
    a waveform sample in the RawSample binary array, similar to the
    ...
    execution time. The audiocore.RawSample.sample_rate frequency will be
    proportional to the original algorithm's PWM frequency output value, likely
    calculated from the sample_rate divided by the length of the
